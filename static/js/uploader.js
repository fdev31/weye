function uploader(input, options) {
    ...
    // add extra, dynamic, post data
    this.fillData = function(data) {
        if ($this.settings.extra_data_func) {
            var d = $this.settings.extra_data_func(data);
            for(k in d) {
                console.log('extra', k, d[k]);
                data.append(String(k), d[k]);
            }
        }
    };
